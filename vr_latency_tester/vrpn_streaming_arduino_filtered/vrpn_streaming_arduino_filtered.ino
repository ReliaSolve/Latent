// ReliaSolve filtered streaming Arduino code to go along with the 
// vrpn_streaming_arduino device.  This is a modified version of
// the vrpn_streaming_arduino program from VRPN.
//
// Copyright 2015
// Author: Russell Taylor working through Reliasolve.com
// LICENSE: Apache 2.0.

// This program differs from the original in that it does multiple
// reads from the analog inputs above 0 and throws away the results
// to reduce the impact of having high output impedence on measurements
// of devices like unbuffered phototransistors with 500kOhm series
// resistors.
...
//*****************************************************
{
  ...
  if (numAnalogs > 0) {
    ...
    for (int i = 1; i < numAnalogs; i++) {
      Serial.print(",");
      // Do a number of reads on the higher channels, throwing out
      // the results.  This is giving the capacitor on the sample-and-
      // hold circuit on the Arduino time to charge through its 10kOhm
      // resistor when it is being fed from a high-impedence device.
      // Channel 0 (the potentiometer) does not require this delay.
      // A phototransistor with a series resistance of 500 kOhms took
      // around 1ms to settle, so we do ten reads here.
      analogRead(i);
      analogRead(i);
      analogRead(i);
      analogRead(i);
